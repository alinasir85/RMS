...
const DateRangeDropdown = (props: any) => {
  ...
  const [dateRangeText, setDateRangeText] = useState<string>('Current Week');
  const dateRangeDropdownRef = useRef<HTMLDivElement>(null);
  const DATE_RANGES : { [key: string]: any } = {
    "Current Week" : "CW",
    "Last Week" : "LW",
    "Last 15 Days" : "LF"
  }
  ...
  useEffect(()=>{
    if(props.onChange){
      props.onChange(DATE_RANGES[dateRangeText]);
      setCollapsed(true);
    }
  },[dateRangeText]);
  ...
  return (
    <div className="daterange-picker-container position-relative" ref={dateRangeDropdownRef}>
      {/* <h3>{dateRangeText}</h3> */}
      <I2cButton className="dateranger-picker-opener" onClick={toggleDataRange} size="large">
        {dateRangeText} <i className="ms-2"><ArrowDown2 size="12" color="#222" /></i>
      </I2cButton>
      {
        props.isFlex ?
        <div className={`daterange-picker p-3 ${collapsed ? 'daterange-picker-collapsed' : ''}`}>
          <div>
            <I2cButton className={`w-100 mb-3 border-0 ${dateRangeText==='Current Week' ? 'active' : ''}`} size="large"
              onClick={() => {
                setDateRangeText('Current Week');
              }}> Current Week </I2cButton>
            <I2cButton className={`w-100 mb-3 border-0 ${dateRangeText==='Last Week' ? 'active' : ''}`} size="large"
              onClick={() => {
                setDateRangeText('Last Week');
              }}> Last Week </I2cButton>
            <I2cButton className={`w-100 border-0 ${dateRangeText==='Last 15 Days' ? 'active' : ''}`} size="large"
              onClick={() => {
        ...
        <div className={`daterange-picker p-4 ${collapsed ? 'daterange-picker-collapsed' : ''}`}>
          <div className='d-flex mb-3'>
            <I2cButton className={`w-100 me-2 ${dateRangeText==='Current Week' ? 'active' : ''}`} size="large"
              onClick={() => {
                setShowDateRange(false);
                setDateRangeText('Current Week');
              }}> Current Week </I2cButton>
            <I2cButton className={`w-100 me-2 ${dateRangeText==='Last Week' ? 'active' : ''}`} size="large"
              onClick={() => {
                setShowDateRange(false);
                setDateRangeText('Last Week');
              }}> Last Week </I2cButton>
            <I2cButton className={`w-100 ${dateRangeText==='Last 15 Days' ? 'active' : ''}`} size="large"
              onClick={() => {
                setShowDateRange(false);
                setDateRangeText('Last 15 Days');
              }}> Last 15 Days </I2cButton>
          ...
          <div className="mb-3 d-flex">
            <I2cButton className={`w-100 ${(dateRangeText!=='Current Week' && dateRangeText!=='Last Week' && dateRangeText!=='Last 15 Days') ? 'active' : ''}`} size="large"
              onClick={() => {
          ...
          </div>
          {
            ...
            <>
              ...
              <div className="text-center date-ranger-footer-buttons d-flex">
                  <I2cButton className='w-100 me-2' size="large"  onClick={cancelDataRangeHandler}> Cancel </I2cButton>
